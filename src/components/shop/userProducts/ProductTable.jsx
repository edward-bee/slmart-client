...
import { getAllUserProduct, deleteProduct } from "./FetchApi";
import moment from "moment";
import { ProductContext } from "./index";
import { fetchUserData } from "./UserId";
...
const AllProduct = (props) => {
  ...
  const fetchData = async () => {
    ...
    console.log(`fetching UID:${userId}`);
    setLoading(true);
    let responseData = await getAllUserProduct(userId);
    setTimeout(() => {
  ...
  const deleteProductReq = async (pId) => {
    ...
    // console.log(`delete p id: ${uid}`);
    let deleteC = await deleteProduct(pId, uId);
    if (deleteC.error) {
  ...
  return (
    <Fragment>
      <div className="col-span-1 overflow-auto bg-white shadow-lg p-4" style={{borderRadius: 15}}>
